<?php
/*
Plugin Name: WP Code Prettify X
...
Change log: added use of span=nocode
*/
...
function WPCP_ContentOld($content) {
...
public function WPCP_Content($content) {
...
public function parse_content_span($matches)
{
	$tags_open = $matches[1];
	$code = $matches[2];
	$tags_close = $matches[3];
	// replace with thing
	return 'startnocode' . htmlentities($tags_open . $code . $tags_close) . 'endnocode';
}

public function parse_content_span_restore($matches)
{
	$code = $matches[1];
...
public function parse_content_pre($matches) {
	...
	if(!empty($matches)) {
		...
		// do a call back for each found span
		$regex2 = '/(<span.*?>)(.*?)(<\/span>)/si';
$regex2 = '/(<(?:span|abbr).*?>)(.*?)(<\/(?:span|abbr)>)/si';
		$code = preg_replace_callback($regex2, array($this, 'parse_content_span'), $code);
	}
	...
	*/
	...
	$html_entities_match = array( "|\<br \/\>|", "#\<#", "#\>#", "|/|", '#\[#', '#\]#', '#"#', "#'#" );
	$html_entities_replace = array( "\n", '&lt;', '&gt;', '&#47;', '&#91;', '&#93;', "&#34;", '&#39;');
	$parsed_code = preg_replace( $html_entities_match, $html_entities_replace, $code );

	$regex = '/startnocode(.*?)endnocode/si';
	$parsed_code = preg_replace_callback($regex, array($this, 'parse_content_span_restore'), $parsed_code);
...
public function parse_content_code($matches) {
	...
	*/
	...
	$html_entities_match = array( "|\<br \/\>|", "#\<#", "#\>#", "|/|", '#\[#', '#\]#', '#"#', "#'#" );
	$html_entities_replace = array( "\n", '&lt;', '&gt;', '&#47;', '&#91;', '&#93;', "&#34;", '&#39;');
	$parsed_code2 = preg_replace( $html_entities_match, $html_entities_replace, $code );
